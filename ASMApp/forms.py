...
from django.contrib.auth.forms import UserCreationForm #this is deault form present in django admin site
from django.contrib.auth.forms import PasswordChangeForm

class UsForm(UserCreationForm): #if it is our form we use model.forms if it is default we use usercreationform
...
class TprofileForm(forms.ModelForm):
	class Meta:
		...
		"tgr":forms.Select(attrs={ #we used dropdown for gender so we use select
			"class":"form-control my-2",
			}),
