...
def register(request):
	if request.method == "POST":
		...
		if s.is_valid():
			h = s.save(commit=False)#commit=false to not save premanantly
...
def updpf(request):
	if request.user.role_type == 2:
		...
		mx = []
		for i in x:
			mx.append(i.tch_id)#appending id
		if request.user.id not in mx: #if id is not exist then only we are posting
			if request.method == "POST":
			...
			if request.method == "POST":
				...
				if c.is_valid():
					...
					return redirect('/pfe')
			c = TprofileForm(instance=z)#z will be accessed
...
def tsgnlist(request):
	...
	if request.method == "POST":
		...
		if y.is_valid():
			h = y.save(commit=False) # we have to set status=1 we gave default 0 in models.py so we assigned to another variable
			h.astatus = 1
...
@login_required
def staslist(request):#displaying assignments according to year
